...
class AdminBerandaLib
{
  ...
  public function getAllday($data)
  {
    $allday = array();
    for ($i = 0; $i < 7; $i++) {
  ...
  public function getAllTransaksi($data)
  {
    ...
    $totaltransaksi['acc'] = count($data['weeknow']['vacc']);
    return $totaltransaksi;
  }

  public function getBonus($data, $nama)
  {
    $bhpintotal       = 0;
    $bhpinreturn      = 0;
    $ballin           = array();
  ...
  public function getselectindex($data, $tipeitem)
  {
    if ($tipeitem == 'servis') {
      ...
      foreach ($data['menuadmin']['sales'] as $value) {
        if ($data['detailitem']['teknisi'] == $value['sales']) {
          $returndata = $index;
        }
        $index += 1;
      }
    } else {
      $returndata = array(0, 0, 0, 0, 0, 0); // itemtoselect jaringan garansi ram rom kelengkapan sales
      for ($i = 0; $i < 6; $i++) {
        $index   = 0;
        $ismatch = false;
